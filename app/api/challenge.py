...
def select_new_challenge(
    ...
    **챌린지 기회 획득 방법:**
    - 일기 작성 시 챌린지 기회 +1 (available_challenges)
    - 챌린지 기회는 일기 작성일 기준 다음날 자정까지만 유효
    - 예: 1/23 일기 작성 → 1/24 자정까지 유효 → 1/25 되면 리셋
    - 최대 2개까지 누적 가능 (어제+오늘 일기를 모두 썼을 경우)
    - 2일 이상 일기를 안 쓰면 챌린지 기회는 0으로 리셋

    **챌린지 선택 조건:**
    - available_challenges가 1 이상이어야 선택 가능
    - 미완료 챌린지가 있으면 새로운 챌린지 선택 불가
    - 미완료 챌린지는 다음날 자정에 자동 삭제됨

    **챌린지 타입:**
    - basic: 대륙당 최대 5개, 중복 선택 불가
    - recommend: 오늘/어제 일기의 AI 추천 기반 (book/music/food 자동 결정)

    **선택 후:**
    - available_challenges -1
    - 대륙별 10개 완료 시 자동으로 다음 대륙 선택 가능
    """
    try:
        user = db.query(User).filter(User.user_id == current_user_id).first()
