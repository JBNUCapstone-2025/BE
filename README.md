...
- 일기 연동 (일기 완료 시 챌린지 기회 획득, 오늘/어제 일기만)
...
### 일기 API
```
POST   /diary/                        - 일기 작성 (title, content, diary_date만)
POST   /diary/{diary_id}/complete     - 일기 완료 (AI 감정 분석 및 추천 + 챌린지 기회 증가)
...
PUT    /diary/{diary_id}              - 일기 수정 (complete 전에만 가능, title/content만)
```
...
### 챌린지 시스템
- 일기 완료 시 챌린지 기회 자동 획득 (오늘/어제 일기만, 최대 2개까지 누적)
